...
to park-in-garage [gateway] ;; procedure to park in garage
  ...
  if (count cars-on current-garage / count current-garage) < 1[
    ...
    ifelse (min-util <= compute-utility current-garage nav-goal)  ;; might fail, could not test until garages are also taken into acc. if it does fail change the condition to wtp >=
    [
