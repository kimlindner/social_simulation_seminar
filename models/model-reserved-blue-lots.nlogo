...
to setup-globals
  ...
  set reserved-blue-lot-current-fee 0 ;; greta1
  ...
  set reserved-blue-lot-current-occup 0 ;; greta1
...
to setup-lots;;intialize dynamic lots
  ...
  set reserved-blue-lot no-patches ;; greta1
  ...
  foreach lot-distances [lot-distance ->
    ...
    if i > lot-count * 0.6[
      set reserved-blue-lot (patch-set reserved-blue-lot patches with [lot-id != 0 and center-distance = lot-distance]) ;; greta1
    ]
    set i i + 1
  ]

  ;; create patch-set for all parking spaces on the curbside
  set lots (patch-set yellow-lot teal-lot green-lot reserved-blue-lot) ;; greta1
...
to record-globals ;; keep track of all global reporter variables
  ...
  set reserved-blue-lot-current-fee mean [fee] of reserved-blue-lot ;; greta1
  ...
  set reserved-blue-lot-current-occup count cars-on reserved-blue-lot / count reserved-blue-lot ;; greta1
...
to control-lots
  if ticks > 0 and (ticks mod (temporal-resolution / controls-per-hour) = 0) [
    ...
    (ifelse
      ...
      switch = 3[
        let potential-offenders cars-on reserved-blue-lot ;; greta1
        let fines (count potential-offenders with [not paid?]) * fines-multiplier * mean [fee] of reserved-blue-lot ;; greta1
        set city-income city-income + fines
        set total-fines total-fines + fines
    ])
  ]
